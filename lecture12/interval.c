...
char *convert_time(time_t t) {
    ...
    //return c_time_string; // c_time has an internal storage
    return strdup(c_time_string);   // copy to new chunk of memory
}
...
Interval *make_interval(time_t start, time_t end) {
    Interval *res = NULL;
    res = (Interval *)malloc(sizeof(Interval));
    if (res == NULL) {
...
double interval_duration(Interval *interval) {
    //double d = interval->end - interval->start;
    //return d;
